...
fun CGPA(semesters : MutableList<Semester>){
    ...
    var credit4 by remember { mutableStateOf<Int?>(null) } //? null safety operator
        ...
        Row() {
            Column(modifier = Modifier.fillMaxHeight(),
                ...
                Button(onClick = { /*TODO*/
                    ...
                    val totalGradePoint = semesters.sumOf { calcGradePoints(it.grade , it.credit) }
                    if(totalCredit > 0 ){
                        cgpa = ((totalGradePoint / totalCredit.toDouble()).toString())
                    }
                    else{
                        cgpa = 0.0.toString()
                    }
...
fun calcGradePoints(grade: String, credit: Int) : Double {
...
fun CreditTextField(credit: Int?, onValueChange: (Int?) -> Unit) {
    TextField(
        value = credit?.toString() ?: "", // Use 'credit' here as the initial value
        onValueChange = { text ->
...
//fun CGPAPreview(){
//    CGPAappTheme {
//        val semesters = mutableListOf<Semester>()  // Create an empty list for preview
//        CGPA(semesters)
//    }
//}
